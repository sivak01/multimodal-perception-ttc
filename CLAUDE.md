...
This is not a git repository. There is no test suite, linter, or build system — correctness is checked via the notebooks themselves (see Step 7 below) and by re-running stages.
...
```bash
jupyter nbconvert --to notebook --execute --inplace "Step_0_Dataset_Preparation_Upgraded_v2.ipynb"
```
...
|---|---|---|
| 0 | `Step_0_Dataset_Preparation_Upgraded_v2.ipynb` | Indexes nuScenes samples; writes `samples_index.json`, the master index every downstream step reads |
| 1.1 | `Step_1_1_Camera_Preprocessing_Upgraded_1.ipynb` | Camera calibration/intrinsics per sample; defines `project_point_to_camera()`, the shared 3D→image projection utility reused in Steps 2.3.1, 3.3, 6 |
| 1.2 | `Step_1_2_Radar_Parsing_Upgraded.ipynb` | Parses raw radar point clouds per channel |
| 1.3 | `Step_1_3_LiDAR_Parsing_Upgraded.ipynb` | Parses raw LiDAR point clouds (N×7 arrays: point + derived fields) |
| 2.1 | `Step_2_1_LiDAR_Processing_Upgraded.ipynb` | Ground-plane removal (Open3D `segment_plane`) + DBSCAN clustering |
| 2.2 | `Step_2_2_Radar_Processing_Upgraded.ipynb` | Radar filtering (dynamic-property filter) + feature extraction (raw + ego-compensated velocity) |
| 2.3 | `Step_2_3_YOLOv5_Upgraded.ipynb` | YOLO 2D object detection on camera frames (uses `ultralytics`, weights in `yolov8n.pt`) |
| 2.3.1 | `Step_2_3_1_YOLO_Global_Projection_NEW.ipynb` | Lifts each 2D YOLO box to a 3D global-frame position by fusing with LiDAR |
| 3.1 | `Step_3_1_LiDAR_Fusion_Upgraded.ipynb` | Multi-frame LiDAR tracking (nearest-neighbor / Hungarian assignment via `scipy.optimize.linear_sum_assignment`), global frame, no filter yet |
| 3.2 | `Step_3_2_Radar_Fusion_Upgraded.ipynb` | Multi-frame radar tracking, same family as 3.1 |
| 3.3 | `Step_3_3_Camera_Tracker_Upgraded.ipynb` | Multi-frame camera tracking, global 3D nearest-neighbor across all 6 cameras |
| 4 | `Step_4_Fusion_Upgraded.ipynb` | Fuses LiDAR/radar/camera tracks into unified tracks |
| 5 | `Step_5_TTC_Upgraded.ipynb` | UKF + CTRV motion model → TTC estimate per track, per sensor and fused |
| 6 | `Step_6_Visualization_Upgraded.ipynb` | Trend plots + evaluation metrics against nuScenes `sample_annotation` ground truth |
...
| — | `Validate_Step_2_3_1_Upgraded.ipynb` | Standalone validator for Step 2.3.1's 3D projections |
